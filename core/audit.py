...
"""Reusable audit logging components for authorized local assessments."""
...
class AuditEvent:
    """One normalized audit logging observation."""
    event_id: str = ""
    action: str = ""
    target: str = ""
    outcome: str = ""
    ...
    def with_metadata(self, **values: Any) -> "AuditEvent":
...
class AuditEventCollection:
    ...
    def get(self, identity: str) -> AuditEvent | None: return self._index.get(identity)

    def filter(self, **criteria: Any) -> list[AuditEvent]:
        return [item for item in self._values if all(getattr(item, key, None) == expected for key, expected in criteria.items())]

    def sort_by(self, field_name: str, reverse: bool = False) -> list[AuditEvent]:
    ...
    def __iter__(self) -> Iterator[AuditEvent]: return iter(tuple(self._values))
    def __getitem__(self, index: int) -> AuditEvent: return self._values[index]
    ...
    @classmethod
    def load_json(cls, path: Path) -> "AuditEventCollection":
        if not path.exists(): return cls()
        raw = json.loads(path.read_text(encoding="utf-8")); return cls(AuditEvent.from_dict(item) for item in raw)

    @classmethod
    def load_jsonl(cls, path: Path) -> "AuditEventCollection":
        if not path.exists(): return cls()
        values = []
        for line in path.read_text(encoding="utf-8").splitlines():
            if line.strip(): values.append(AuditEvent.from_dict(json.loads(line)))
        return cls(values)

    def merge(self, other: "AuditEventCollection") -> "AuditEventCollection":
        result = AuditEventCollection(self._values); result.extend(other); return result
...
class AuditEventParser:
    ...
    def parse_mapping(self, value: Mapping[str, Any]) -> AuditEvent:
        return AuditEvent.from_dict(value)

    def parse_many(self, values: Iterable[Mapping[str, Any]]) -> AuditEventCollection:
        return AuditEventCollection(self.parse_mapping(value) for value in values)

    def parse_lines(self, text: str, delimiter: str = "|") -> AuditEventCollection:
        rows = []
        names = list(AuditEvent.__dataclass_fields__.keys())
        for line in text.splitlines():
            ...
            rows.append(AuditEvent.from_dict(row))
        return AuditEventCollection(rows)

    def normalize(self, value: AuditEvent) -> AuditEvent:
        data = value.to_dict()
...
class AuditEventValidator:
    ...
    def validate(self, value: AuditEvent) -> list[str]:
        errors = []
    ...
    def require_valid(self, value: AuditEvent) -> AuditEvent:
        errors = self.validate(value)
        if errors: raise ValueError("; ".join(errors))
        return value

    def validate_collection(self, values: AuditEventCollection) -> dict[str, list[str]]:
...
class AuditEventIndex:
    ...
    def export_summary(self) -> dict[str, Any]:
        return {"records": len(self.values), "identities": len(self.by_identity), "fields": list(AuditEvent.__dataclass_fields__)}


def load_events(path: str | Path) -> AuditEventCollection:
    return AuditEventCollection.load_json(Path(path))


def save_events(values: AuditEventCollection, path: str | Path) -> Path:
    return values.save_json(Path(path))


def build_events(**values: Any) -> AuditEvent:
    return AuditEvent.from_dict(values)
