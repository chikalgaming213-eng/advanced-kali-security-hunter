...
"""Reusable pause resume checkpoints components for authorized local assessments."""
...
class Checkpoint:
    """One normalized pause resume checkpoints observation."""
    job_id: str = ""
    step: str = ""
    progress: float | int | str = 0
    state: str = ""
    ...
    def with_metadata(self, **values: Any) -> "Checkpoint":
...
class CheckpointCollection:
    ...
    def get(self, identity: str) -> Checkpoint | None: return self._index.get(identity)

    def filter(self, **criteria: Any) -> list[Checkpoint]:
        return [item for item in self._values if all(getattr(item, key, None) == expected for key, expected in criteria.items())]

    def sort_by(self, field_name: str, reverse: bool = False) -> list[Checkpoint]:
    ...
    def __iter__(self) -> Iterator[Checkpoint]: return iter(tuple(self._values))
    def __getitem__(self, index: int) -> Checkpoint: return self._values[index]
    ...
    @classmethod
    def load_json(cls, path: Path) -> "CheckpointCollection":
        if not path.exists(): return cls()
        raw = json.loads(path.read_text(encoding="utf-8")); return cls(Checkpoint.from_dict(item) for item in raw)

    @classmethod
    def load_jsonl(cls, path: Path) -> "CheckpointCollection":
        if not path.exists(): return cls()
        values = []
        for line in path.read_text(encoding="utf-8").splitlines():
            if line.strip(): values.append(Checkpoint.from_dict(json.loads(line)))
        return cls(values)

    def merge(self, other: "CheckpointCollection") -> "CheckpointCollection":
        result = CheckpointCollection(self._values); result.extend(other); return result
...
class CheckpointParser:
    ...
    def parse_mapping(self, value: Mapping[str, Any]) -> Checkpoint:
        return Checkpoint.from_dict(value)

    def parse_many(self, values: Iterable[Mapping[str, Any]]) -> CheckpointCollection:
        return CheckpointCollection(self.parse_mapping(value) for value in values)

    def parse_lines(self, text: str, delimiter: str = "|") -> CheckpointCollection:
        rows = []
        names = list(Checkpoint.__dataclass_fields__.keys())
        for line in text.splitlines():
            ...
            rows.append(Checkpoint.from_dict(row))
        return CheckpointCollection(rows)

    def normalize(self, value: Checkpoint) -> Checkpoint:
        data = value.to_dict()
...
class CheckpointValidator:
    ...
    def validate(self, value: Checkpoint) -> list[str]:
        errors = []
    ...
    def require_valid(self, value: Checkpoint) -> Checkpoint:
        errors = self.validate(value)
        if errors: raise ValueError("; ".join(errors))
        return value

    def validate_collection(self, values: CheckpointCollection) -> dict[str, list[str]]:
...
class CheckpointIndex:
    ...
    def export_summary(self) -> dict[str, Any]:
        return {"records": len(self.values), "identities": len(self.by_identity), "fields": list(Checkpoint.__dataclass_fields__)}


def load_checkpoints(path: str | Path) -> CheckpointCollection:
    return CheckpointCollection.load_json(Path(path))


def save_checkpoints(values: CheckpointCollection, path: str | Path) -> Path:
    return values.save_json(Path(path))


def build_checkpoints(**values: Any) -> Checkpoint:
    return Checkpoint.from_dict(values)
