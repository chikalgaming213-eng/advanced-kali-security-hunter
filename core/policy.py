...
"""Reusable security policy evaluation components for authorized local assessments."""
...
class PolicyRule:
    """One normalized security policy evaluation observation."""
    name: str = ""
    enabled: bool = False
    rationale: str = ""
    severity: str = ""
...
class PolicyRuleCollection:
    ...
    def get(self, identity: str) -> PolicyRule | None: return self._index.get(identity)

    def filter(self, **criteria: Any) -> list[PolicyRule]:
        return [item for item in self._values if all(getattr(item, key, None) == expected for key, expected in criteria.items())]

    def sort_by(self, field_name: str, reverse: bool = False) -> list[PolicyRule]:
    ...
    def __iter__(self) -> Iterator[PolicyRule]: return iter(tuple(self._values))
    def __getitem__(self, index: int) -> PolicyRule: return self._values[index]
    ...
    @classmethod
    def load_json(cls, path: Path) -> "PolicyRuleCollection":
        if not path.exists(): return cls()
        raw = json.loads(path.read_text(encoding="utf-8")); return cls(PolicyRule.from_dict(item) for item in raw)

    @classmethod
    def load_jsonl(cls, path: Path) -> "PolicyRuleCollection":
        if not path.exists(): return cls()
        values = []
        for line in path.read_text(encoding="utf-8").splitlines():
            if line.strip(): values.append(PolicyRule.from_dict(json.loads(line)))
        return cls(values)

    def merge(self, other: "PolicyRuleCollection") -> "PolicyRuleCollection":
        result = PolicyRuleCollection(self._values); result.extend(other); return result
...
class PolicyRuleParser:
    ...
    def parse_mapping(self, value: Mapping[str, Any]) -> PolicyRule:
        return PolicyRule.from_dict(value)

    def parse_many(self, values: Iterable[Mapping[str, Any]]) -> PolicyRuleCollection:
        return PolicyRuleCollection(self.parse_mapping(value) for value in values)

    def parse_lines(self, text: str, delimiter: str = "|") -> PolicyRuleCollection:
        rows = []
        names = list(PolicyRule.__dataclass_fields__.keys())
        for line in text.splitlines():
            ...
            rows.append(PolicyRule.from_dict(row))
        return PolicyRuleCollection(rows)

    def normalize(self, value: PolicyRule) -> PolicyRule:
        data = value.to_dict()
...
class PolicyRuleValidator:
    ...
    def validate(self, value: PolicyRule) -> list[str]:
        errors = []
    ...
    def require_valid(self, value: PolicyRule) -> PolicyRule:
        errors = self.validate(value)
        if errors: raise ValueError("; ".join(errors))
        return value

    def validate_collection(self, values: PolicyRuleCollection) -> dict[str, list[str]]:
...
class PolicyRuleIndex:
    ...
    def __init__(self, values: PolicyRuleCollection) -> None:
        self.values = values
    ...
    def group(self, field_name: str) -> dict[str, list[PolicyRule]]:
    ...
    def export_summary(self) -> dict[str, Any]:
        return {"records": len(self.values), "identities": len(self.by_identity), "fields": list(PolicyRule.__dataclass_fields__)}


def load_rules(path: str | Path) -> PolicyRuleCollection:
    return PolicyRuleCollection.load_json(Path(path))


def save_rules(values: PolicyRuleCollection, path: str | Path) -> Path:
    return values.save_json(Path(path))


def build_rules(**values: Any) -> PolicyRule:
    return PolicyRule.from_dict(values)
