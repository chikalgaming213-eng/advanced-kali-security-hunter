...
"""Reusable bounded retry policy components for authorized local assessments."""
...
class RetryAttempt:
    """One normalized bounded retry policy observation."""
    operation: str = ""
    attempt: float | int | str = 0
    ...
    @classmethod
    def from_dict(cls, value: Mapping[str, Any]) -> "RetryAttempt":
...
class RetryAttemptCollection:
    ...
    def get(self, identity: str) -> RetryAttempt | None: return self._index.get(identity)

    def filter(self, **criteria: Any) -> list[RetryAttempt]:
        return [item for item in self._values if all(getattr(item, key, None) == expected for key, expected in criteria.items())]

    def sort_by(self, field_name: str, reverse: bool = False) -> list[RetryAttempt]:
    ...
    def __iter__(self) -> Iterator[RetryAttempt]: return iter(tuple(self._values))
    def __getitem__(self, index: int) -> RetryAttempt: return self._values[index]
    ...
    @classmethod
    def load_json(cls, path: Path) -> "RetryAttemptCollection":
        if not path.exists(): return cls()
        raw = json.loads(path.read_text(encoding="utf-8")); return cls(RetryAttempt.from_dict(item) for item in raw)

    @classmethod
    def load_jsonl(cls, path: Path) -> "RetryAttemptCollection":
        if not path.exists(): return cls()
        values = []
        for line in path.read_text(encoding="utf-8").splitlines():
            if line.strip(): values.append(RetryAttempt.from_dict(json.loads(line)))
        return cls(values)

    def merge(self, other: "RetryAttemptCollection") -> "RetryAttemptCollection":
        result = RetryAttemptCollection(self._values); result.extend(other); return result
...
class RetryAttemptParser:
    """Conservative parser accepting mappings or delimited text."""
    def parse_mapping(self, value: Mapping[str, Any]) -> RetryAttempt:
        return RetryAttempt.from_dict(value)

    def parse_many(self, values: Iterable[Mapping[str, Any]]) -> RetryAttemptCollection:
        return RetryAttemptCollection(self.parse_mapping(value) for value in values)

    def parse_lines(self, text: str, delimiter: str = "|") -> RetryAttemptCollection:
        rows = []
        names = list(RetryAttempt.__dataclass_fields__.keys())
        for line in text.splitlines():
            ...
            rows.append(RetryAttempt.from_dict(row))
        return RetryAttemptCollection(rows)

    def normalize(self, value: RetryAttempt) -> RetryAttempt:
        data = value.to_dict()
        for key, item in list(data.items()):
            if isinstance(item, str): data[key] = item.strip()
        return RetryAttempt.from_dict(data)

class RetryAttemptValidator:
    """Validation rules shared by adapters and report generators."""
    def validate(self, value: RetryAttempt) -> list[str]:
        errors = []
    ...
    def require_valid(self, value: RetryAttempt) -> RetryAttempt:
        errors = self.validate(value)
        if errors: raise ValueError("; ".join(errors))
        return value

    def validate_collection(self, values: RetryAttemptCollection) -> dict[str, list[str]]:
        return {value.identity(): self.validate(value) for value in values if self.validate(value)}

class RetryAttemptIndex:
    """Small query index for deterministic local analysis."""
    def __init__(self, values: RetryAttemptCollection) -> None:
        self.values = values
        self.by_identity = {value.identity(): value for value in values}

    def search(self, query: str) -> list[RetryAttempt]:
        needle = query.casefold(); return [value for value in self.values if needle in json.dumps(value.to_dict()).casefold()]

    def group(self, field_name: str) -> dict[str, list[RetryAttempt]]:
        groups: dict[str, list[RetryAttempt]] = {}
        for value in self.values:
            key = str(getattr(value, field_name, "")); groups.setdefault(key, []).append(value)
        return groups

    def export_summary(self) -> dict[str, Any]:
        return {"records": len(self.values), "identities": len(self.by_identity), "fields": list(RetryAttempt.__dataclass_fields__)}


def load_attempts(path: str | Path) -> RetryAttemptCollection:
    return RetryAttemptCollection.load_json(Path(path))


def save_attempts(values: RetryAttemptCollection, path: str | Path) -> Path:
    return values.save_json(Path(path))


def build_attempts(**values: Any) -> RetryAttempt:
    return RetryAttempt.from_dict(values)
