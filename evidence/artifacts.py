...
"""Reusable evidence collection components for authorized local assessments."""
...
class EvidenceArtifact:
    """One normalized evidence collection observation."""
    artifact_id: str = ""
    path: str = ""
    sha256: str = ""
    description: str = ""
...
class EvidenceArtifactCollection:
    ...
    def get(self, identity: str) -> EvidenceArtifact | None: return self._index.get(identity)

    def filter(self, **criteria: Any) -> list[EvidenceArtifact]:
        return [item for item in self._values if all(getattr(item, key, None) == expected for key, expected in criteria.items())]

    def sort_by(self, field_name: str, reverse: bool = False) -> list[EvidenceArtifact]:
    ...
    def __iter__(self) -> Iterator[EvidenceArtifact]: return iter(tuple(self._values))
    def __getitem__(self, index: int) -> EvidenceArtifact: return self._values[index]
    ...
    @classmethod
    def load_json(cls, path: Path) -> "EvidenceArtifactCollection":
        if not path.exists(): return cls()
        raw = json.loads(path.read_text(encoding="utf-8")); return cls(EvidenceArtifact.from_dict(item) for item in raw)

    @classmethod
    def load_jsonl(cls, path: Path) -> "EvidenceArtifactCollection":
        if not path.exists(): return cls()
        values = []
        for line in path.read_text(encoding="utf-8").splitlines():
            if line.strip(): values.append(EvidenceArtifact.from_dict(json.loads(line)))
        return cls(values)

    def merge(self, other: "EvidenceArtifactCollection") -> "EvidenceArtifactCollection":
        result = EvidenceArtifactCollection(self._values); result.extend(other); return result
...
class EvidenceArtifactParser:
    """Conservative parser accepting mappings or delimited text."""
    def parse_mapping(self, value: Mapping[str, Any]) -> EvidenceArtifact:
        return EvidenceArtifact.from_dict(value)

    def parse_many(self, values: Iterable[Mapping[str, Any]]) -> EvidenceArtifactCollection:
        return EvidenceArtifactCollection(self.parse_mapping(value) for value in values)

    def parse_lines(self, text: str, delimiter: str = "|") -> EvidenceArtifactCollection:
        rows = []
        names = list(EvidenceArtifact.__dataclass_fields__.keys())
        for line in text.splitlines():
            ...
            rows.append(EvidenceArtifact.from_dict(row))
        return EvidenceArtifactCollection(rows)

    def normalize(self, value: EvidenceArtifact) -> EvidenceArtifact:
        data = value.to_dict()
        for key, item in list(data.items()):
            if isinstance(item, str): data[key] = item.strip()
        return EvidenceArtifact.from_dict(data)

class EvidenceArtifactValidator:
    """Validation rules shared by adapters and report generators."""
    def validate(self, value: EvidenceArtifact) -> list[str]:
        errors = []
    ...
    def require_valid(self, value: EvidenceArtifact) -> EvidenceArtifact:
        errors = self.validate(value)
        if errors: raise ValueError("; ".join(errors))
        return value

    def validate_collection(self, values: EvidenceArtifactCollection) -> dict[str, list[str]]:
...
class EvidenceArtifactIndex:
    ...
    def __init__(self, values: EvidenceArtifactCollection) -> None:
        self.values = values
    ...
    def group(self, field_name: str) -> dict[str, list[EvidenceArtifact]]:
    ...
    def export_summary(self) -> dict[str, Any]:
        return {"records": len(self.values), "identities": len(self.by_identity), "fields": list(EvidenceArtifact.__dataclass_fields__)}


def load_artifacts(path: str | Path) -> EvidenceArtifactCollection:
    return EvidenceArtifactCollection.load_json(Path(path))


def save_artifacts(values: EvidenceArtifactCollection, path: str | Path) -> Path:
    return values.save_json(Path(path))


def build_artifacts(**values: Any) -> EvidenceArtifact:
    return EvidenceArtifact.from_dict(values)
