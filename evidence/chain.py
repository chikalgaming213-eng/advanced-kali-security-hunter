...
"""Reusable evidence chain of custody components for authorized local assessments."""
...
class ChainEntry:
    """One normalized evidence chain of custody observation."""
    artifact_id: str = ""
    action: str = ""
    actor: str = ""
    timestamp: str = ""
    ...
    @classmethod
    def from_dict(cls, value: Mapping[str, Any]) -> "ChainEntry":
...
class ChainEntryCollection:
    ...
    def get(self, identity: str) -> ChainEntry | None: return self._index.get(identity)

    def filter(self, **criteria: Any) -> list[ChainEntry]:
        return [item for item in self._values if all(getattr(item, key, None) == expected for key, expected in criteria.items())]

    def sort_by(self, field_name: str, reverse: bool = False) -> list[ChainEntry]:
    ...
    def __iter__(self) -> Iterator[ChainEntry]: return iter(tuple(self._values))
    def __getitem__(self, index: int) -> ChainEntry: return self._values[index]
    ...
    @classmethod
    def load_json(cls, path: Path) -> "ChainEntryCollection":
        if not path.exists(): return cls()
        raw = json.loads(path.read_text(encoding="utf-8")); return cls(ChainEntry.from_dict(item) for item in raw)

    @classmethod
    def load_jsonl(cls, path: Path) -> "ChainEntryCollection":
        if not path.exists(): return cls()
        values = []
        for line in path.read_text(encoding="utf-8").splitlines():
            if line.strip(): values.append(ChainEntry.from_dict(json.loads(line)))
        return cls(values)

    def merge(self, other: "ChainEntryCollection") -> "ChainEntryCollection":
        result = ChainEntryCollection(self._values); result.extend(other); return result
...
class ChainEntryParser:
    """Conservative parser accepting mappings or delimited text."""
    def parse_mapping(self, value: Mapping[str, Any]) -> ChainEntry:
        return ChainEntry.from_dict(value)

    def parse_many(self, values: Iterable[Mapping[str, Any]]) -> ChainEntryCollection:
        return ChainEntryCollection(self.parse_mapping(value) for value in values)

    def parse_lines(self, text: str, delimiter: str = "|") -> ChainEntryCollection:
        rows = []
        names = list(ChainEntry.__dataclass_fields__.keys())
        for line in text.splitlines():
            ...
            rows.append(ChainEntry.from_dict(row))
        return ChainEntryCollection(rows)

    def normalize(self, value: ChainEntry) -> ChainEntry:
        data = value.to_dict()
        for key, item in list(data.items()):
            if isinstance(item, str): data[key] = item.strip()
        return ChainEntry.from_dict(data)

class ChainEntryValidator:
    """Validation rules shared by adapters and report generators."""
    def validate(self, value: ChainEntry) -> list[str]:
        errors = []
    ...
    def require_valid(self, value: ChainEntry) -> ChainEntry:
        errors = self.validate(value)
        if errors: raise ValueError("; ".join(errors))
        return value

    def validate_collection(self, values: ChainEntryCollection) -> dict[str, list[str]]:
        return {value.identity(): self.validate(value) for value in values if self.validate(value)}

class ChainEntryIndex:
    """Small query index for deterministic local analysis."""
    def __init__(self, values: ChainEntryCollection) -> None:
        self.values = values
    ...
    def group(self, field_name: str) -> dict[str, list[ChainEntry]]:
        groups: dict[str, list[ChainEntry]] = {}
        for value in self.values:
            key = str(getattr(value, field_name, "")); groups.setdefault(key, []).append(value)
        return groups

    def export_summary(self) -> dict[str, Any]:
        return {"records": len(self.values), "identities": len(self.by_identity), "fields": list(ChainEntry.__dataclass_fields__)}


def load_entries(path: str | Path) -> ChainEntryCollection:
    return ChainEntryCollection.load_json(Path(path))


def save_entries(values: ChainEntryCollection, path: str | Path) -> Path:
    return values.save_json(Path(path))


def build_entries(**values: Any) -> ChainEntry:
    return ChainEntry.from_dict(values)
