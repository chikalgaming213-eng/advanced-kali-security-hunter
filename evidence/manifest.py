...
"""Reusable evidence manifest components for authorized local assessments."""
...
class ManifestItem:
    """One normalized evidence manifest observation."""
    path: str = ""
    size: float | int | str = 0
    digest: str = ""
    media_type: str = ""
    ...
    @classmethod
    def from_dict(cls, value: Mapping[str, Any]) -> "ManifestItem":
    ...
    def with_metadata(self, **values: Any) -> "ManifestItem":
...
class ManifestItemCollection:
    ...
    def __init__(self, values: Iterable[ManifestItem] = ()) -> None:
        self._values: list[ManifestItem] = []
        self._index: dict[str, ManifestItem] = {}
        for value in values: self.add(value)

    def add(self, value: ManifestItem) -> bool:
        key = value.identity()
        if key in self._index: return False
        self._index[key] = value; self._values.append(value); return True

    def extend(self, values: Iterable[ManifestItem]) -> int:
        count = 0
    ...
    def get(self, identity: str) -> ManifestItem | None: return self._index.get(identity)

    def filter(self, **criteria: Any) -> list[ManifestItem]:
        return [item for item in self._values if all(getattr(item, key, None) == expected for key, expected in criteria.items())]

    def sort_by(self, field_name: str, reverse: bool = False) -> list[ManifestItem]:
    ...
    def __iter__(self) -> Iterator[ManifestItem]: return iter(tuple(self._values))
    def __getitem__(self, index: int) -> ManifestItem: return self._values[index]
    ...
    @classmethod
    def load_json(cls, path: Path) -> "ManifestItemCollection":
        if not path.exists(): return cls()
        raw = json.loads(path.read_text(encoding="utf-8")); return cls(ManifestItem.from_dict(item) for item in raw)

    @classmethod
    def load_jsonl(cls, path: Path) -> "ManifestItemCollection":
        if not path.exists(): return cls()
        values = []
        for line in path.read_text(encoding="utf-8").splitlines():
            if line.strip(): values.append(ManifestItem.from_dict(json.loads(line)))
        return cls(values)

    def merge(self, other: "ManifestItemCollection") -> "ManifestItemCollection":
        result = ManifestItemCollection(self._values); result.extend(other); return result
...
class ManifestItemParser:
    """Conservative parser accepting mappings or delimited text."""
    def parse_mapping(self, value: Mapping[str, Any]) -> ManifestItem:
        return ManifestItem.from_dict(value)

    def parse_many(self, values: Iterable[Mapping[str, Any]]) -> ManifestItemCollection:
        return ManifestItemCollection(self.parse_mapping(value) for value in values)

    def parse_lines(self, text: str, delimiter: str = "|") -> ManifestItemCollection:
        rows = []
        names = list(ManifestItem.__dataclass_fields__.keys())
        for line in text.splitlines():
            ...
            rows.append(ManifestItem.from_dict(row))
        return ManifestItemCollection(rows)

    def normalize(self, value: ManifestItem) -> ManifestItem:
        data = value.to_dict()
        for key, item in list(data.items()):
            if isinstance(item, str): data[key] = item.strip()
        return ManifestItem.from_dict(data)

class ManifestItemValidator:
    """Validation rules shared by adapters and report generators."""
    def validate(self, value: ManifestItem) -> list[str]:
        errors = []
...
class ManifestItemIndex:
    ...
    def export_summary(self) -> dict[str, Any]:
        return {"records": len(self.values), "identities": len(self.by_identity), "fields": list(ManifestItem.__dataclass_fields__)}


def load_items(path: str | Path) -> ManifestItemCollection:
    return ManifestItemCollection.load_json(Path(path))


def save_items(values: ManifestItemCollection, path: str | Path) -> Path:
    return values.save_json(Path(path))


def build_items(**values: Any) -> ManifestItem:
    return ManifestItem.from_dict(values)
