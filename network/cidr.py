...
"""Reusable CIDR expansion components for authorized local assessments."""
...
class NetworkHost:
    """One normalized CIDR expansion observation."""
    address: str = ""
    network: str = ""
    ...
    @classmethod
    def from_dict(cls, value: Mapping[str, Any]) -> "NetworkHost":
...
class NetworkHostCollection:
    ...
    def get(self, identity: str) -> NetworkHost | None: return self._index.get(identity)

    def filter(self, **criteria: Any) -> list[NetworkHost]:
        return [item for item in self._values if all(getattr(item, key, None) == expected for key, expected in criteria.items())]

    def sort_by(self, field_name: str, reverse: bool = False) -> list[NetworkHost]:
    ...
    def __iter__(self) -> Iterator[NetworkHost]: return iter(tuple(self._values))
    def __getitem__(self, index: int) -> NetworkHost: return self._values[index]
    ...
    @classmethod
    def load_json(cls, path: Path) -> "NetworkHostCollection":
        if not path.exists(): return cls()
        raw = json.loads(path.read_text(encoding="utf-8")); return cls(NetworkHost.from_dict(item) for item in raw)

    @classmethod
    def load_jsonl(cls, path: Path) -> "NetworkHostCollection":
        if not path.exists(): return cls()
        values = []
        for line in path.read_text(encoding="utf-8").splitlines():
            if line.strip(): values.append(NetworkHost.from_dict(json.loads(line)))
        return cls(values)

    def merge(self, other: "NetworkHostCollection") -> "NetworkHostCollection":
        result = NetworkHostCollection(self._values); result.extend(other); return result
...
class NetworkHostParser:
    """Conservative parser accepting mappings or delimited text."""
    def parse_mapping(self, value: Mapping[str, Any]) -> NetworkHost:
        return NetworkHost.from_dict(value)

    def parse_many(self, values: Iterable[Mapping[str, Any]]) -> NetworkHostCollection:
        return NetworkHostCollection(self.parse_mapping(value) for value in values)

    def parse_lines(self, text: str, delimiter: str = "|") -> NetworkHostCollection:
        rows = []
        names = list(NetworkHost.__dataclass_fields__.keys())
        for line in text.splitlines():
            ...
            rows.append(NetworkHost.from_dict(row))
        return NetworkHostCollection(rows)

    def normalize(self, value: NetworkHost) -> NetworkHost:
        data = value.to_dict()
        for key, item in list(data.items()):
            if isinstance(item, str): data[key] = item.strip()
        return NetworkHost.from_dict(data)

class NetworkHostValidator:
    """Validation rules shared by adapters and report generators."""
    def validate(self, value: NetworkHost) -> list[str]:
        errors = []
    ...
    def require_valid(self, value: NetworkHost) -> NetworkHost:
        errors = self.validate(value)
        if errors: raise ValueError("; ".join(errors))
        return value

    def validate_collection(self, values: NetworkHostCollection) -> dict[str, list[str]]:
        return {value.identity(): self.validate(value) for value in values if self.validate(value)}

class NetworkHostIndex:
    """Small query index for deterministic local analysis."""
    def __init__(self, values: NetworkHostCollection) -> None:
        self.values = values
        self.by_identity = {value.identity(): value for value in values}

    def search(self, query: str) -> list[NetworkHost]:
        needle = query.casefold(); return [value for value in self.values if needle in json.dumps(value.to_dict()).casefold()]

    def group(self, field_name: str) -> dict[str, list[NetworkHost]]:
        groups: dict[str, list[NetworkHost]] = {}
        for value in self.values:
            key = str(getattr(value, field_name, "")); groups.setdefault(key, []).append(value)
        return groups

    def export_summary(self) -> dict[str, Any]:
        return {"records": len(self.values), "identities": len(self.by_identity), "fields": list(NetworkHost.__dataclass_fields__)}


def load_hosts(path: str | Path) -> NetworkHostCollection:
    return NetworkHostCollection.load_json(Path(path))


def save_hosts(values: NetworkHostCollection, path: str | Path) -> Path:
    return values.save_json(Path(path))


def build_hosts(**values: Any) -> NetworkHost:
    return NetworkHost.from_dict(values)
