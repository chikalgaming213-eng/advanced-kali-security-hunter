...
"""Reusable port discovery components for authorized local assessments."""
...
class Port:
    """One normalized port discovery observation."""
    ...
    def with_metadata(self, **values: Any) -> "Port":
...
class PortCollection:
    ...
    def __init__(self, values: Iterable[Port] = ()) -> None:
        self._values: list[Port] = []
        self._index: dict[str, Port] = {}
        for value in values: self.add(value)

    def add(self, value: Port) -> bool:
        key = value.identity()
        if key in self._index: return False
        self._index[key] = value; self._values.append(value); return True

    def extend(self, values: Iterable[Port]) -> int:
        count = 0
    ...
    def get(self, identity: str) -> Port | None: return self._index.get(identity)

    def filter(self, **criteria: Any) -> list[Port]:
        return [item for item in self._values if all(getattr(item, key, None) == expected for key, expected in criteria.items())]

    def sort_by(self, field_name: str, reverse: bool = False) -> list[Port]:
    ...
    def __iter__(self) -> Iterator[Port]: return iter(tuple(self._values))
    def __getitem__(self, index: int) -> Port: return self._values[index]
    ...
    @classmethod
    def load_json(cls, path: Path) -> "PortCollection":
        if not path.exists(): return cls()
        raw = json.loads(path.read_text(encoding="utf-8")); return cls(Port.from_dict(item) for item in raw)

    @classmethod
    def load_jsonl(cls, path: Path) -> "PortCollection":
        if not path.exists(): return cls()
        values = []
        for line in path.read_text(encoding="utf-8").splitlines():
            if line.strip(): values.append(Port.from_dict(json.loads(line)))
        return cls(values)

    def merge(self, other: "PortCollection") -> "PortCollection":
        result = PortCollection(self._values); result.extend(other); return result
...
class PortParser:
    ...
    def parse_mapping(self, value: Mapping[str, Any]) -> Port:
        return Port.from_dict(value)

    def parse_many(self, values: Iterable[Mapping[str, Any]]) -> PortCollection:
        return PortCollection(self.parse_mapping(value) for value in values)

    def parse_lines(self, text: str, delimiter: str = "|") -> PortCollection:
        rows = []
        names = list(Port.__dataclass_fields__.keys())
        for line in text.splitlines():
            ...
            rows.append(Port.from_dict(row))
        return PortCollection(rows)

    def normalize(self, value: Port) -> Port:
        data = value.to_dict()
...
class PortValidator:
    ...
    def validate(self, value: Port) -> list[str]:
        errors = []
    ...
    def require_valid(self, value: Port) -> Port:
        errors = self.validate(value)
        if errors: raise ValueError("; ".join(errors))
        return value

    def validate_collection(self, values: PortCollection) -> dict[str, list[str]]:
...
class PortIndex:
    ...
    def __init__(self, values: PortCollection) -> None:
        self.values = values
    ...
    def group(self, field_name: str) -> dict[str, list[Port]]:
    ...
    def export_summary(self) -> dict[str, Any]:
        return {"records": len(self.values), "identities": len(self.by_identity), "fields": list(Port.__dataclass_fields__)}


def load_ports(path: str | Path) -> PortCollection:
    return PortCollection.load_json(Path(path))


def save_ports(values: PortCollection, path: str | Path) -> Path:
    return values.save_json(Path(path))


def build_ports(**values: Any) -> Port:
    return Port.from_dict(values)
