...
"""Reusable rate limiting components for authorized local assessments."""
...
class RateSample:
    """One normalized rate limiting observation."""
    target: str = ""
    timestamp: str = ""
    allowed: bool = False
    ...
    @classmethod
    def from_dict(cls, value: Mapping[str, Any]) -> "RateSample":
...
class RateSampleCollection:
    ...
    def get(self, identity: str) -> RateSample | None: return self._index.get(identity)

    def filter(self, **criteria: Any) -> list[RateSample]:
        return [item for item in self._values if all(getattr(item, key, None) == expected for key, expected in criteria.items())]

    def sort_by(self, field_name: str, reverse: bool = False) -> list[RateSample]:
    ...
    def __iter__(self) -> Iterator[RateSample]: return iter(tuple(self._values))
    def __getitem__(self, index: int) -> RateSample: return self._values[index]
    ...
    @classmethod
    def load_json(cls, path: Path) -> "RateSampleCollection":
        if not path.exists(): return cls()
        raw = json.loads(path.read_text(encoding="utf-8")); return cls(RateSample.from_dict(item) for item in raw)

    @classmethod
    def load_jsonl(cls, path: Path) -> "RateSampleCollection":
        if not path.exists(): return cls()
        values = []
        for line in path.read_text(encoding="utf-8").splitlines():
            if line.strip(): values.append(RateSample.from_dict(json.loads(line)))
        return cls(values)

    def merge(self, other: "RateSampleCollection") -> "RateSampleCollection":
        result = RateSampleCollection(self._values); result.extend(other); return result
...
class RateSampleParser:
    """Conservative parser accepting mappings or delimited text."""
    def parse_mapping(self, value: Mapping[str, Any]) -> RateSample:
        return RateSample.from_dict(value)

    def parse_many(self, values: Iterable[Mapping[str, Any]]) -> RateSampleCollection:
        return RateSampleCollection(self.parse_mapping(value) for value in values)

    def parse_lines(self, text: str, delimiter: str = "|") -> RateSampleCollection:
        rows = []
        names = list(RateSample.__dataclass_fields__.keys())
        for line in text.splitlines():
            ...
            rows.append(RateSample.from_dict(row))
        return RateSampleCollection(rows)

    def normalize(self, value: RateSample) -> RateSample:
        data = value.to_dict()
        for key, item in list(data.items()):
            if isinstance(item, str): data[key] = item.strip()
        return RateSample.from_dict(data)

class RateSampleValidator:
    """Validation rules shared by adapters and report generators."""
    def validate(self, value: RateSample) -> list[str]:
        errors = []
    ...
    def require_valid(self, value: RateSample) -> RateSample:
        errors = self.validate(value)
        if errors: raise ValueError("; ".join(errors))
        return value

    def validate_collection(self, values: RateSampleCollection) -> dict[str, list[str]]:
        return {value.identity(): self.validate(value) for value in values if self.validate(value)}

class RateSampleIndex:
    """Small query index for deterministic local analysis."""
    def __init__(self, values: RateSampleCollection) -> None:
        self.values = values
        self.by_identity = {value.identity(): value for value in values}

    def search(self, query: str) -> list[RateSample]:
        needle = query.casefold(); return [value for value in self.values if needle in json.dumps(value.to_dict()).casefold()]

    def group(self, field_name: str) -> dict[str, list[RateSample]]:
        groups: dict[str, list[RateSample]] = {}
        for value in self.values:
            key = str(getattr(value, field_name, "")); groups.setdefault(key, []).append(value)
        return groups

    def export_summary(self) -> dict[str, Any]:
        return {"records": len(self.values), "identities": len(self.by_identity), "fields": list(RateSample.__dataclass_fields__)}


def load_rate_samples(path: str | Path) -> RateSampleCollection:
    return RateSampleCollection.load_json(Path(path))


def save_rate_samples(values: RateSampleCollection, path: str | Path) -> Path:
    return values.save_json(Path(path))


def build_rate_samples(**values: Any) -> RateSample:
    return RateSample.from_dict(values)
