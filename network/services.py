...
"""Reusable service detection components for authorized local assessments."""
...
class Service:
    """One normalized service detection observation."""
    port: float | int | str = 0
    name: str = ""
    product: str = ""
    version: str = ""
    ...
    @classmethod
    def from_dict(cls, value: Mapping[str, Any]) -> "Service":
    ...
    def with_metadata(self, **values: Any) -> "Service":
...
class ServiceCollection:
    ...
    def __init__(self, values: Iterable[Service] = ()) -> None:
        self._values: list[Service] = []
        self._index: dict[str, Service] = {}
        for value in values: self.add(value)

    def add(self, value: Service) -> bool:
        key = value.identity()
        if key in self._index: return False
        self._index[key] = value; self._values.append(value); return True

    def extend(self, values: Iterable[Service]) -> int:
        count = 0
    ...
    def get(self, identity: str) -> Service | None: return self._index.get(identity)

    def filter(self, **criteria: Any) -> list[Service]:
        return [item for item in self._values if all(getattr(item, key, None) == expected for key, expected in criteria.items())]

    def sort_by(self, field_name: str, reverse: bool = False) -> list[Service]:
    ...
    def __iter__(self) -> Iterator[Service]: return iter(tuple(self._values))
    def __getitem__(self, index: int) -> Service: return self._values[index]
    ...
    @classmethod
    def load_json(cls, path: Path) -> "ServiceCollection":
        if not path.exists(): return cls()
        raw = json.loads(path.read_text(encoding="utf-8")); return cls(Service.from_dict(item) for item in raw)

    @classmethod
    def load_jsonl(cls, path: Path) -> "ServiceCollection":
        if not path.exists(): return cls()
        values = []
        for line in path.read_text(encoding="utf-8").splitlines():
            if line.strip(): values.append(Service.from_dict(json.loads(line)))
        return cls(values)

    def merge(self, other: "ServiceCollection") -> "ServiceCollection":
        result = ServiceCollection(self._values); result.extend(other); return result
...
class ServiceParser:
    ...
    def parse_mapping(self, value: Mapping[str, Any]) -> Service:
        return Service.from_dict(value)

    def parse_many(self, values: Iterable[Mapping[str, Any]]) -> ServiceCollection:
        return ServiceCollection(self.parse_mapping(value) for value in values)

    def parse_lines(self, text: str, delimiter: str = "|") -> ServiceCollection:
        rows = []
        names = list(Service.__dataclass_fields__.keys())
        for line in text.splitlines():
            ...
            rows.append(Service.from_dict(row))
        return ServiceCollection(rows)

    def normalize(self, value: Service) -> Service:
        data = value.to_dict()
...
class ServiceValidator:
    ...
    def validate(self, value: Service) -> list[str]:
        errors = []
    ...
    def require_valid(self, value: Service) -> Service:
        errors = self.validate(value)
        if errors: raise ValueError("; ".join(errors))
        return value

    def validate_collection(self, values: ServiceCollection) -> dict[str, list[str]]:
...
class ServiceIndex:
    ...
    def __init__(self, values: ServiceCollection) -> None:
        self.values = values
    ...
    def group(self, field_name: str) -> dict[str, list[Service]]:
    ...
    def export_summary(self) -> dict[str, Any]:
        return {"records": len(self.values), "identities": len(self.by_identity), "fields": list(Service.__dataclass_fields__)}


def load_services(path: str | Path) -> ServiceCollection:
    return ServiceCollection.load_json(Path(path))


def save_services(values: ServiceCollection, path: str | Path) -> Path:
    return values.save_json(Path(path))


def build_services(**values: Any) -> Service:
    return Service.from_dict(values)
