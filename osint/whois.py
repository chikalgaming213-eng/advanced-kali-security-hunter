...
"""Reusable WHOIS normalization components for authorized local assessments."""
...
class WhoisField:
    """One normalized WHOIS normalization observation."""
    name: str = ""
    ...
    @classmethod
    def from_dict(cls, value: Mapping[str, Any]) -> "WhoisField":
...
class WhoisFieldCollection:
    ...
    def get(self, identity: str) -> WhoisField | None: return self._index.get(identity)

    def filter(self, **criteria: Any) -> list[WhoisField]:
        return [item for item in self._values if all(getattr(item, key, None) == expected for key, expected in criteria.items())]

    def sort_by(self, field_name: str, reverse: bool = False) -> list[WhoisField]:
    ...
    def __iter__(self) -> Iterator[WhoisField]: return iter(tuple(self._values))
    def __getitem__(self, index: int) -> WhoisField: return self._values[index]
    ...
    @classmethod
    def load_json(cls, path: Path) -> "WhoisFieldCollection":
        if not path.exists(): return cls()
        raw = json.loads(path.read_text(encoding="utf-8")); return cls(WhoisField.from_dict(item) for item in raw)

    @classmethod
    def load_jsonl(cls, path: Path) -> "WhoisFieldCollection":
        if not path.exists(): return cls()
        values = []
        for line in path.read_text(encoding="utf-8").splitlines():
            if line.strip(): values.append(WhoisField.from_dict(json.loads(line)))
        return cls(values)

    def merge(self, other: "WhoisFieldCollection") -> "WhoisFieldCollection":
        result = WhoisFieldCollection(self._values); result.extend(other); return result
...
class WhoisFieldParser:
    """Conservative parser accepting mappings or delimited text."""
    def parse_mapping(self, value: Mapping[str, Any]) -> WhoisField:
        return WhoisField.from_dict(value)

    def parse_many(self, values: Iterable[Mapping[str, Any]]) -> WhoisFieldCollection:
        return WhoisFieldCollection(self.parse_mapping(value) for value in values)

    def parse_lines(self, text: str, delimiter: str = "|") -> WhoisFieldCollection:
        rows = []
        names = list(WhoisField.__dataclass_fields__.keys())
        for line in text.splitlines():
            ...
            rows.append(WhoisField.from_dict(row))
        return WhoisFieldCollection(rows)

    def normalize(self, value: WhoisField) -> WhoisField:
        data = value.to_dict()
        for key, item in list(data.items()):
            if isinstance(item, str): data[key] = item.strip()
        return WhoisField.from_dict(data)

class WhoisFieldValidator:
    """Validation rules shared by adapters and report generators."""
    def validate(self, value: WhoisField) -> list[str]:
        errors = []
    ...
    def require_valid(self, value: WhoisField) -> WhoisField:
        errors = self.validate(value)
        if errors: raise ValueError("; ".join(errors))
        return value

    def validate_collection(self, values: WhoisFieldCollection) -> dict[str, list[str]]:
        return {value.identity(): self.validate(value) for value in values if self.validate(value)}

class WhoisFieldIndex:
    """Small query index for deterministic local analysis."""
    def __init__(self, values: WhoisFieldCollection) -> None:
        self.values = values
        self.by_identity = {value.identity(): value for value in values}

    def search(self, query: str) -> list[WhoisField]:
        needle = query.casefold(); return [value for value in self.values if needle in json.dumps(value.to_dict()).casefold()]

    def group(self, field_name: str) -> dict[str, list[WhoisField]]:
        groups: dict[str, list[WhoisField]] = {}
        for value in self.values:
            key = str(getattr(value, field_name, "")); groups.setdefault(key, []).append(value)
        return groups

    def export_summary(self) -> dict[str, Any]:
        return {"records": len(self.values), "identities": len(self.by_identity), "fields": list(WhoisField.__dataclass_fields__)}


def load_fields(path: str | Path) -> WhoisFieldCollection:
    return WhoisFieldCollection.load_json(Path(path))


def save_fields(values: WhoisFieldCollection, path: str | Path) -> Path:
    return values.save_json(Path(path))


def build_fields(**values: Any) -> WhoisField:
    return WhoisField.from_dict(values)
