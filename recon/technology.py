...
"""Reusable technology detection components for authorized local assessments."""
...
class Technology:
    """One normalized technology detection observation."""
    name: str = ""
    version: str = ""
    evidence: str = ""
    ...
    @classmethod
    def from_dict(cls, value: Mapping[str, Any]) -> "Technology":
    ...
    def with_metadata(self, **values: Any) -> "Technology":
...
class TechnologyCollection:
    ...
    def __init__(self, values: Iterable[Technology] = ()) -> None:
        self._values: list[Technology] = []
        self._index: dict[str, Technology] = {}
        for value in values: self.add(value)

    def add(self, value: Technology) -> bool:
        key = value.identity()
        if key in self._index: return False
        self._index[key] = value; self._values.append(value); return True

    def extend(self, values: Iterable[Technology]) -> int:
        count = 0
    ...
    def get(self, identity: str) -> Technology | None: return self._index.get(identity)

    def filter(self, **criteria: Any) -> list[Technology]:
        return [item for item in self._values if all(getattr(item, key, None) == expected for key, expected in criteria.items())]

    def sort_by(self, field_name: str, reverse: bool = False) -> list[Technology]:
    ...
    def __iter__(self) -> Iterator[Technology]: return iter(tuple(self._values))
    def __getitem__(self, index: int) -> Technology: return self._values[index]
    ...
    @classmethod
    def load_json(cls, path: Path) -> "TechnologyCollection":
        if not path.exists(): return cls()
        raw = json.loads(path.read_text(encoding="utf-8")); return cls(Technology.from_dict(item) for item in raw)

    @classmethod
    def load_jsonl(cls, path: Path) -> "TechnologyCollection":
        if not path.exists(): return cls()
        values = []
        for line in path.read_text(encoding="utf-8").splitlines():
            if line.strip(): values.append(Technology.from_dict(json.loads(line)))
        return cls(values)

    def merge(self, other: "TechnologyCollection") -> "TechnologyCollection":
        result = TechnologyCollection(self._values); result.extend(other); return result
...
class TechnologyParser:
    ...
    def parse_mapping(self, value: Mapping[str, Any]) -> Technology:
        return Technology.from_dict(value)

    def parse_many(self, values: Iterable[Mapping[str, Any]]) -> TechnologyCollection:
        return TechnologyCollection(self.parse_mapping(value) for value in values)

    def parse_lines(self, text: str, delimiter: str = "|") -> TechnologyCollection:
        rows = []
        names = list(Technology.__dataclass_fields__.keys())
        for line in text.splitlines():
            ...
            rows.append(Technology.from_dict(row))
        return TechnologyCollection(rows)

    def normalize(self, value: Technology) -> Technology:
        data = value.to_dict()
        for key, item in list(data.items()):
            if isinstance(item, str): data[key] = item.strip()
        return Technology.from_dict(data)

class TechnologyValidator:
    """Validation rules shared by adapters and report generators."""
    def validate(self, value: Technology) -> list[str]:
        errors = []
    ...
    def require_valid(self, value: Technology) -> Technology:
        errors = self.validate(value)
        if errors: raise ValueError("; ".join(errors))
        return value

    def validate_collection(self, values: TechnologyCollection) -> dict[str, list[str]]:
...
class TechnologyIndex:
    ...
    def __init__(self, values: TechnologyCollection) -> None:
        self.values = values
        self.by_identity = {value.identity(): value for value in values}

    def search(self, query: str) -> list[Technology]:
        needle = query.casefold(); return [value for value in self.values if needle in json.dumps(value.to_dict()).casefold()]

    def group(self, field_name: str) -> dict[str, list[Technology]]:
        groups: dict[str, list[Technology]] = {}
        for value in self.values:
            key = str(getattr(value, field_name, "")); groups.setdefault(key, []).append(value)
        return groups

    def export_summary(self) -> dict[str, Any]:
        return {"records": len(self.values), "identities": len(self.by_identity), "fields": list(Technology.__dataclass_fields__)}


def load_technologies(path: str | Path) -> TechnologyCollection:
    return TechnologyCollection.load_json(Path(path))


def save_technologies(values: TechnologyCollection, path: str | Path) -> Path:
    return values.save_json(Path(path))


def build_technologies(**values: Any) -> Technology:
    return Technology.from_dict(values)
