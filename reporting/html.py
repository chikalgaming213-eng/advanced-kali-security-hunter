...
"""Reusable HTML reporting components for authorized local assessments."""
...
class HtmlFinding:
    """One normalized HTML reporting observation."""
    finding_id: str = ""
    title: str = ""
    severity: str = ""
    target: str = ""
    ...
    @classmethod
    def from_dict(cls, value: Mapping[str, Any]) -> "HtmlFinding":
...
class HtmlFindingCollection:
    ...
    def get(self, identity: str) -> HtmlFinding | None: return self._index.get(identity)

    def filter(self, **criteria: Any) -> list[HtmlFinding]:
        return [item for item in self._values if all(getattr(item, key, None) == expected for key, expected in criteria.items())]

    def sort_by(self, field_name: str, reverse: bool = False) -> list[HtmlFinding]:
    ...
    def __iter__(self) -> Iterator[HtmlFinding]: return iter(tuple(self._values))
    def __getitem__(self, index: int) -> HtmlFinding: return self._values[index]
    ...
    @classmethod
    def load_json(cls, path: Path) -> "HtmlFindingCollection":
        if not path.exists(): return cls()
        raw = json.loads(path.read_text(encoding="utf-8")); return cls(HtmlFinding.from_dict(item) for item in raw)

    @classmethod
    def load_jsonl(cls, path: Path) -> "HtmlFindingCollection":
        if not path.exists(): return cls()
        values = []
        for line in path.read_text(encoding="utf-8").splitlines():
            if line.strip(): values.append(HtmlFinding.from_dict(json.loads(line)))
        return cls(values)

    def merge(self, other: "HtmlFindingCollection") -> "HtmlFindingCollection":
        result = HtmlFindingCollection(self._values); result.extend(other); return result
...
class HtmlFindingParser:
    """Conservative parser accepting mappings or delimited text."""
    def parse_mapping(self, value: Mapping[str, Any]) -> HtmlFinding:
        return HtmlFinding.from_dict(value)

    def parse_many(self, values: Iterable[Mapping[str, Any]]) -> HtmlFindingCollection:
        return HtmlFindingCollection(self.parse_mapping(value) for value in values)

    def parse_lines(self, text: str, delimiter: str = "|") -> HtmlFindingCollection:
        rows = []
        names = list(HtmlFinding.__dataclass_fields__.keys())
        for line in text.splitlines():
            ...
            rows.append(HtmlFinding.from_dict(row))
        return HtmlFindingCollection(rows)

    def normalize(self, value: HtmlFinding) -> HtmlFinding:
        data = value.to_dict()
        for key, item in list(data.items()):
            if isinstance(item, str): data[key] = item.strip()
        return HtmlFinding.from_dict(data)

class HtmlFindingValidator:
    """Validation rules shared by adapters and report generators."""
    def validate(self, value: HtmlFinding) -> list[str]:
        errors = []
    ...
    def require_valid(self, value: HtmlFinding) -> HtmlFinding:
        errors = self.validate(value)
        if errors: raise ValueError("; ".join(errors))
        return value

    def validate_collection(self, values: HtmlFindingCollection) -> dict[str, list[str]]:
        return {value.identity(): self.validate(value) for value in values if self.validate(value)}

class HtmlFindingIndex:
    """Small query index for deterministic local analysis."""
    def __init__(self, values: HtmlFindingCollection) -> None:
        self.values = values
        self.by_identity = {value.identity(): value for value in values}

    def search(self, query: str) -> list[HtmlFinding]:
        needle = query.casefold(); return [value for value in self.values if needle in json.dumps(value.to_dict()).casefold()]

    def group(self, field_name: str) -> dict[str, list[HtmlFinding]]:
        groups: dict[str, list[HtmlFinding]] = {}
        for value in self.values:
            key = str(getattr(value, field_name, "")); groups.setdefault(key, []).append(value)
        return groups

    def export_summary(self) -> dict[str, Any]:
        return {"records": len(self.values), "identities": len(self.by_identity), "fields": list(HtmlFinding.__dataclass_fields__)}


def load_findings(path: str | Path) -> HtmlFindingCollection:
    return HtmlFindingCollection.load_json(Path(path))


def save_findings(values: HtmlFindingCollection, path: str | Path) -> Path:
    return values.save_json(Path(path))


def build_findings(**values: Any) -> HtmlFinding:
    return HtmlFinding.from_dict(values)
