...
"""Reusable JSONL persistence components for authorized local assessments."""
...
class JsonlRecord:
    """One normalized JSONL persistence observation."""
    record_type: str = ""
    payload: str = ""
    timestamp: str = ""
    ...
    @classmethod
    def from_dict(cls, value: Mapping[str, Any]) -> "JsonlRecord":
...
class JsonlRecordCollection:
    ...
    def get(self, identity: str) -> JsonlRecord | None: return self._index.get(identity)

    def filter(self, **criteria: Any) -> list[JsonlRecord]:
        return [item for item in self._values if all(getattr(item, key, None) == expected for key, expected in criteria.items())]

    def sort_by(self, field_name: str, reverse: bool = False) -> list[JsonlRecord]:
    ...
    def __iter__(self) -> Iterator[JsonlRecord]: return iter(tuple(self._values))
    def __getitem__(self, index: int) -> JsonlRecord: return self._values[index]
    ...
    @classmethod
    def load_json(cls, path: Path) -> "JsonlRecordCollection":
        if not path.exists(): return cls()
        raw = json.loads(path.read_text(encoding="utf-8")); return cls(JsonlRecord.from_dict(item) for item in raw)

    @classmethod
    def load_jsonl(cls, path: Path) -> "JsonlRecordCollection":
        if not path.exists(): return cls()
        values = []
        for line in path.read_text(encoding="utf-8").splitlines():
            if line.strip(): values.append(JsonlRecord.from_dict(json.loads(line)))
        return cls(values)

    def merge(self, other: "JsonlRecordCollection") -> "JsonlRecordCollection":
        result = JsonlRecordCollection(self._values); result.extend(other); return result
...
class JsonlRecordParser:
    ...
    def parse_mapping(self, value: Mapping[str, Any]) -> JsonlRecord:
        return JsonlRecord.from_dict(value)

    def parse_many(self, values: Iterable[Mapping[str, Any]]) -> JsonlRecordCollection:
        return JsonlRecordCollection(self.parse_mapping(value) for value in values)

    def parse_lines(self, text: str, delimiter: str = "|") -> JsonlRecordCollection:
        rows = []
        names = list(JsonlRecord.__dataclass_fields__.keys())
        for line in text.splitlines():
            ...
            rows.append(JsonlRecord.from_dict(row))
        return JsonlRecordCollection(rows)

    def normalize(self, value: JsonlRecord) -> JsonlRecord:
        data = value.to_dict()
        for key, item in list(data.items()):
            if isinstance(item, str): data[key] = item.strip()
        return JsonlRecord.from_dict(data)

class JsonlRecordValidator:
    """Validation rules shared by adapters and report generators."""
    def validate(self, value: JsonlRecord) -> list[str]:
        errors = []
    ...
    def require_valid(self, value: JsonlRecord) -> JsonlRecord:
        errors = self.validate(value)
        if errors: raise ValueError("; ".join(errors))
        return value

    def validate_collection(self, values: JsonlRecordCollection) -> dict[str, list[str]]:
...
class JsonlRecordIndex:
    ...
    def __init__(self, values: JsonlRecordCollection) -> None:
        self.values = values
        self.by_identity = {value.identity(): value for value in values}

    def search(self, query: str) -> list[JsonlRecord]:
        needle = query.casefold(); return [value for value in self.values if needle in json.dumps(value.to_dict()).casefold()]

    def group(self, field_name: str) -> dict[str, list[JsonlRecord]]:
        groups: dict[str, list[JsonlRecord]] = {}
        for value in self.values:
            key = str(getattr(value, field_name, "")); groups.setdefault(key, []).append(value)
        return groups

    def export_summary(self) -> dict[str, Any]:
        return {"records": len(self.values), "identities": len(self.by_identity), "fields": list(JsonlRecord.__dataclass_fields__)}


def load_records(path: str | Path) -> JsonlRecordCollection:
    return JsonlRecordCollection.load_json(Path(path))


def save_records(values: JsonlRecordCollection, path: str | Path) -> Path:
    return values.save_json(Path(path))


def build_records(**values: Any) -> JsonlRecord:
    return JsonlRecord.from_dict(values)
