...
"""Reusable report summary components for authorized local assessments."""
...
class SummaryMetric:
    """One normalized report summary observation."""
    name: str = ""
    ...
    @classmethod
    def from_dict(cls, value: Mapping[str, Any]) -> "SummaryMetric":
    ...
    def with_metadata(self, **values: Any) -> "SummaryMetric":
...
class SummaryMetricCollection:
    ...
    def __init__(self, values: Iterable[SummaryMetric] = ()) -> None:
        self._values: list[SummaryMetric] = []
        self._index: dict[str, SummaryMetric] = {}
        for value in values: self.add(value)

    def add(self, value: SummaryMetric) -> bool:
        key = value.identity()
        if key in self._index: return False
        self._index[key] = value; self._values.append(value); return True

    def extend(self, values: Iterable[SummaryMetric]) -> int:
        count = 0
    ...
    def get(self, identity: str) -> SummaryMetric | None: return self._index.get(identity)

    def filter(self, **criteria: Any) -> list[SummaryMetric]:
        return [item for item in self._values if all(getattr(item, key, None) == expected for key, expected in criteria.items())]

    def sort_by(self, field_name: str, reverse: bool = False) -> list[SummaryMetric]:
    ...
    def __iter__(self) -> Iterator[SummaryMetric]: return iter(tuple(self._values))
    def __getitem__(self, index: int) -> SummaryMetric: return self._values[index]
    ...
    @classmethod
    def load_json(cls, path: Path) -> "SummaryMetricCollection":
        if not path.exists(): return cls()
        raw = json.loads(path.read_text(encoding="utf-8")); return cls(SummaryMetric.from_dict(item) for item in raw)

    @classmethod
    def load_jsonl(cls, path: Path) -> "SummaryMetricCollection":
        if not path.exists(): return cls()
        values = []
        for line in path.read_text(encoding="utf-8").splitlines():
            if line.strip(): values.append(SummaryMetric.from_dict(json.loads(line)))
        return cls(values)

    def merge(self, other: "SummaryMetricCollection") -> "SummaryMetricCollection":
        result = SummaryMetricCollection(self._values); result.extend(other); return result
...
class SummaryMetricParser:
    """Conservative parser accepting mappings or delimited text."""
    def parse_mapping(self, value: Mapping[str, Any]) -> SummaryMetric:
        return SummaryMetric.from_dict(value)

    def parse_many(self, values: Iterable[Mapping[str, Any]]) -> SummaryMetricCollection:
        return SummaryMetricCollection(self.parse_mapping(value) for value in values)

    def parse_lines(self, text: str, delimiter: str = "|") -> SummaryMetricCollection:
        rows = []
        names = list(SummaryMetric.__dataclass_fields__.keys())
        for line in text.splitlines():
            ...
            rows.append(SummaryMetric.from_dict(row))
        return SummaryMetricCollection(rows)

    def normalize(self, value: SummaryMetric) -> SummaryMetric:
        data = value.to_dict()
        for key, item in list(data.items()):
            if isinstance(item, str): data[key] = item.strip()
        return SummaryMetric.from_dict(data)

class SummaryMetricValidator:
    """Validation rules shared by adapters and report generators."""
    def validate(self, value: SummaryMetric) -> list[str]:
        errors = []
    ...
    def require_valid(self, value: SummaryMetric) -> SummaryMetric:
        errors = self.validate(value)
        if errors: raise ValueError("; ".join(errors))
        return value

    def validate_collection(self, values: SummaryMetricCollection) -> dict[str, list[str]]:
        return {value.identity(): self.validate(value) for value in values if self.validate(value)}

class SummaryMetricIndex:
    """Small query index for deterministic local analysis."""
    def __init__(self, values: SummaryMetricCollection) -> None:
        self.values = values
        self.by_identity = {value.identity(): value for value in values}

    def search(self, query: str) -> list[SummaryMetric]:
        needle = query.casefold(); return [value for value in self.values if needle in json.dumps(value.to_dict()).casefold()]

    def group(self, field_name: str) -> dict[str, list[SummaryMetric]]:
        groups: dict[str, list[SummaryMetric]] = {}
        for value in self.values:
            key = str(getattr(value, field_name, "")); groups.setdefault(key, []).append(value)
        return groups

    def export_summary(self) -> dict[str, Any]:
        return {"records": len(self.values), "identities": len(self.by_identity), "fields": list(SummaryMetric.__dataclass_fields__)}


def load_metrics(path: str | Path) -> SummaryMetricCollection:
    return SummaryMetricCollection.load_json(Path(path))


def save_metrics(values: SummaryMetricCollection, path: str | Path) -> Path:
    return values.save_json(Path(path))


def build_metrics(**values: Any) -> SummaryMetric:
    return SummaryMetric.from_dict(values)
