...
"""Reusable runtime metrics components for authorized local assessments."""
...
class MetricSample:
    """One normalized runtime metrics observation."""
    name: str = ""
    ...
    @classmethod
    def from_dict(cls, value: Mapping[str, Any]) -> "MetricSample":
...
class MetricSampleCollection:
    ...
    def get(self, identity: str) -> MetricSample | None: return self._index.get(identity)

    def filter(self, **criteria: Any) -> list[MetricSample]:
        return [item for item in self._values if all(getattr(item, key, None) == expected for key, expected in criteria.items())]

    def sort_by(self, field_name: str, reverse: bool = False) -> list[MetricSample]:
    ...
    def __iter__(self) -> Iterator[MetricSample]: return iter(tuple(self._values))
    def __getitem__(self, index: int) -> MetricSample: return self._values[index]
    ...
    @classmethod
    def load_json(cls, path: Path) -> "MetricSampleCollection":
        if not path.exists(): return cls()
        raw = json.loads(path.read_text(encoding="utf-8")); return cls(MetricSample.from_dict(item) for item in raw)

    @classmethod
    def load_jsonl(cls, path: Path) -> "MetricSampleCollection":
        if not path.exists(): return cls()
        values = []
        for line in path.read_text(encoding="utf-8").splitlines():
            if line.strip(): values.append(MetricSample.from_dict(json.loads(line)))
        return cls(values)

    def merge(self, other: "MetricSampleCollection") -> "MetricSampleCollection":
        result = MetricSampleCollection(self._values); result.extend(other); return result
...
class MetricSampleParser:
    """Conservative parser accepting mappings or delimited text."""
    def parse_mapping(self, value: Mapping[str, Any]) -> MetricSample:
        return MetricSample.from_dict(value)

    def parse_many(self, values: Iterable[Mapping[str, Any]]) -> MetricSampleCollection:
        return MetricSampleCollection(self.parse_mapping(value) for value in values)

    def parse_lines(self, text: str, delimiter: str = "|") -> MetricSampleCollection:
        rows = []
        names = list(MetricSample.__dataclass_fields__.keys())
        for line in text.splitlines():
            ...
            rows.append(MetricSample.from_dict(row))
        return MetricSampleCollection(rows)

    def normalize(self, value: MetricSample) -> MetricSample:
        data = value.to_dict()
        for key, item in list(data.items()):
            if isinstance(item, str): data[key] = item.strip()
        return MetricSample.from_dict(data)

class MetricSampleValidator:
    """Validation rules shared by adapters and report generators."""
    def validate(self, value: MetricSample) -> list[str]:
        errors = []
...
class MetricSampleIndex:
    ...
    def export_summary(self) -> dict[str, Any]:
        return {"records": len(self.values), "identities": len(self.by_identity), "fields": list(MetricSample.__dataclass_fields__)}


def load_samples(path: str | Path) -> MetricSampleCollection:
    return MetricSampleCollection.load_json(Path(path))


def save_samples(values: MetricSampleCollection, path: str | Path) -> Path:
    return values.save_json(Path(path))


def build_samples(**values: Any) -> MetricSample:
    return MetricSample.from_dict(values)
