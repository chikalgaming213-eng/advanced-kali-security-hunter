...
"""Reusable safe local paths components for authorized local assessments."""
...
class PathRecord:
    """One normalized safe local paths observation."""
    path: str = ""
    exists: bool = False
    readable: bool = False
    kind: str = ""
    ...
    @classmethod
    def from_dict(cls, value: Mapping[str, Any]) -> "PathRecord":
...
class PathRecordCollection:
    ...
    def get(self, identity: str) -> PathRecord | None: return self._index.get(identity)

    def filter(self, **criteria: Any) -> list[PathRecord]:
        return [item for item in self._values if all(getattr(item, key, None) == expected for key, expected in criteria.items())]

    def sort_by(self, field_name: str, reverse: bool = False) -> list[PathRecord]:
    ...
    def __iter__(self) -> Iterator[PathRecord]: return iter(tuple(self._values))
    def __getitem__(self, index: int) -> PathRecord: return self._values[index]
    ...
    @classmethod
    def load_json(cls, path: Path) -> "PathRecordCollection":
        if not path.exists(): return cls()
        raw = json.loads(path.read_text(encoding="utf-8")); return cls(PathRecord.from_dict(item) for item in raw)

    @classmethod
    def load_jsonl(cls, path: Path) -> "PathRecordCollection":
        if not path.exists(): return cls()
        values = []
        for line in path.read_text(encoding="utf-8").splitlines():
            if line.strip(): values.append(PathRecord.from_dict(json.loads(line)))
        return cls(values)

    def merge(self, other: "PathRecordCollection") -> "PathRecordCollection":
        result = PathRecordCollection(self._values); result.extend(other); return result
...
class PathRecordParser:
    """Conservative parser accepting mappings or delimited text."""
    def parse_mapping(self, value: Mapping[str, Any]) -> PathRecord:
        return PathRecord.from_dict(value)

    def parse_many(self, values: Iterable[Mapping[str, Any]]) -> PathRecordCollection:
        return PathRecordCollection(self.parse_mapping(value) for value in values)

    def parse_lines(self, text: str, delimiter: str = "|") -> PathRecordCollection:
        rows = []
        names = list(PathRecord.__dataclass_fields__.keys())
        for line in text.splitlines():
            ...
            rows.append(PathRecord.from_dict(row))
        return PathRecordCollection(rows)

    def normalize(self, value: PathRecord) -> PathRecord:
        data = value.to_dict()
        for key, item in list(data.items()):
            if isinstance(item, str): data[key] = item.strip()
        return PathRecord.from_dict(data)

class PathRecordValidator:
    """Validation rules shared by adapters and report generators."""
    def validate(self, value: PathRecord) -> list[str]:
        errors = []
...
class PathRecordIndex:
    ...
    def export_summary(self) -> dict[str, Any]:
        return {"records": len(self.values), "identities": len(self.by_identity), "fields": list(PathRecord.__dataclass_fields__)}


def load_paths(path: str | Path) -> PathRecordCollection:
    return PathRecordCollection.load_json(Path(path))


def save_paths(values: PathRecordCollection, path: str | Path) -> Path:
    return values.save_json(Path(path))


def build_paths(**values: Any) -> PathRecord:
    return PathRecord.from_dict(values)
