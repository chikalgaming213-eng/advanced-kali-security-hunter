...
"""Reusable robots.txt parsing components for authorized local assessments."""
...
class RobotsRule:
    """One normalized robots.txt parsing observation."""
    path: str = ""
    directive: str = ""
    source: str = ""
    ...
    @classmethod
    def from_dict(cls, value: Mapping[str, Any]) -> "RobotsRule":
...
class RobotsRuleCollection:
    ...
    def get(self, identity: str) -> RobotsRule | None: return self._index.get(identity)

    def filter(self, **criteria: Any) -> list[RobotsRule]:
        return [item for item in self._values if all(getattr(item, key, None) == expected for key, expected in criteria.items())]

    def sort_by(self, field_name: str, reverse: bool = False) -> list[RobotsRule]:
    ...
    def __iter__(self) -> Iterator[RobotsRule]: return iter(tuple(self._values))
    def __getitem__(self, index: int) -> RobotsRule: return self._values[index]
    ...
    @classmethod
    def load_json(cls, path: Path) -> "RobotsRuleCollection":
        if not path.exists(): return cls()
        raw = json.loads(path.read_text(encoding="utf-8")); return cls(RobotsRule.from_dict(item) for item in raw)

    @classmethod
    def load_jsonl(cls, path: Path) -> "RobotsRuleCollection":
        if not path.exists(): return cls()
        values = []
        for line in path.read_text(encoding="utf-8").splitlines():
            if line.strip(): values.append(RobotsRule.from_dict(json.loads(line)))
        return cls(values)

    def merge(self, other: "RobotsRuleCollection") -> "RobotsRuleCollection":
        result = RobotsRuleCollection(self._values); result.extend(other); return result
...
class RobotsRuleParser:
    ...
    def parse_mapping(self, value: Mapping[str, Any]) -> RobotsRule:
        return RobotsRule.from_dict(value)

    def parse_many(self, values: Iterable[Mapping[str, Any]]) -> RobotsRuleCollection:
        return RobotsRuleCollection(self.parse_mapping(value) for value in values)

    def parse_lines(self, text: str, delimiter: str = "|") -> RobotsRuleCollection:
        rows = []
        names = list(RobotsRule.__dataclass_fields__.keys())
        for line in text.splitlines():
            ...
            rows.append(RobotsRule.from_dict(row))
        return RobotsRuleCollection(rows)

    def normalize(self, value: RobotsRule) -> RobotsRule:
        data = value.to_dict()
        for key, item in list(data.items()):
            if isinstance(item, str): data[key] = item.strip()
        return RobotsRule.from_dict(data)

class RobotsRuleValidator:
    """Validation rules shared by adapters and report generators."""
    def validate(self, value: RobotsRule) -> list[str]:
        errors = []
...
class RobotsRuleIndex:
    ...
    def export_summary(self) -> dict[str, Any]:
        return {"records": len(self.values), "identities": len(self.by_identity), "fields": list(RobotsRule.__dataclass_fields__)}


def load_rules(path: str | Path) -> RobotsRuleCollection:
    return RobotsRuleCollection.load_json(Path(path))


def save_rules(values: RobotsRuleCollection, path: str | Path) -> Path:
    return values.save_json(Path(path))


def build_rules(**values: Any) -> RobotsRule:
    return RobotsRule.from_dict(values)
