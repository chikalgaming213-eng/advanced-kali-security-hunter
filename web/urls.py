...
"""Reusable HTTP discovery components for authorized local assessments."""
...
class WebUrl:
    """One normalized HTTP discovery observation."""
    url: str = ""
    status: str = ""
    title: str = ""
    content_type: str = ""
    ...
    @classmethod
    def from_dict(cls, value: Mapping[str, Any]) -> "WebUrl":
...
class WebUrlCollection:
    ...
    def get(self, identity: str) -> WebUrl | None: return self._index.get(identity)

    def filter(self, **criteria: Any) -> list[WebUrl]:
        return [item for item in self._values if all(getattr(item, key, None) == expected for key, expected in criteria.items())]

    def sort_by(self, field_name: str, reverse: bool = False) -> list[WebUrl]:
    ...
    def __iter__(self) -> Iterator[WebUrl]: return iter(tuple(self._values))
    def __getitem__(self, index: int) -> WebUrl: return self._values[index]
    ...
    @classmethod
    def load_json(cls, path: Path) -> "WebUrlCollection":
        if not path.exists(): return cls()
        raw = json.loads(path.read_text(encoding="utf-8")); return cls(WebUrl.from_dict(item) for item in raw)

    @classmethod
    def load_jsonl(cls, path: Path) -> "WebUrlCollection":
        if not path.exists(): return cls()
        values = []
        for line in path.read_text(encoding="utf-8").splitlines():
            if line.strip(): values.append(WebUrl.from_dict(json.loads(line)))
        return cls(values)

    def merge(self, other: "WebUrlCollection") -> "WebUrlCollection":
        result = WebUrlCollection(self._values); result.extend(other); return result
...
class WebUrlParser:
    ...
    def parse_mapping(self, value: Mapping[str, Any]) -> WebUrl:
        return WebUrl.from_dict(value)

    def parse_many(self, values: Iterable[Mapping[str, Any]]) -> WebUrlCollection:
        return WebUrlCollection(self.parse_mapping(value) for value in values)

    def parse_lines(self, text: str, delimiter: str = "|") -> WebUrlCollection:
        rows = []
        names = list(WebUrl.__dataclass_fields__.keys())
        for line in text.splitlines():
            ...
            rows.append(WebUrl.from_dict(row))
        return WebUrlCollection(rows)

    def normalize(self, value: WebUrl) -> WebUrl:
        data = value.to_dict()
        for key, item in list(data.items()):
            if isinstance(item, str): data[key] = item.strip()
        return WebUrl.from_dict(data)

class WebUrlValidator:
    """Validation rules shared by adapters and report generators."""
    def validate(self, value: WebUrl) -> list[str]:
        errors = []
    ...
    def require_valid(self, value: WebUrl) -> WebUrl:
        errors = self.validate(value)
        if errors: raise ValueError("; ".join(errors))
        return value

    def validate_collection(self, values: WebUrlCollection) -> dict[str, list[str]]:
...
class WebUrlIndex:
    ...
    def __init__(self, values: WebUrlCollection) -> None:
        self.values = values
        self.by_identity = {value.identity(): value for value in values}

    def search(self, query: str) -> list[WebUrl]:
        needle = query.casefold(); return [value for value in self.values if needle in json.dumps(value.to_dict()).casefold()]

    def group(self, field_name: str) -> dict[str, list[WebUrl]]:
        groups: dict[str, list[WebUrl]] = {}
        for value in self.values:
            key = str(getattr(value, field_name, "")); groups.setdefault(key, []).append(value)
        return groups

    def export_summary(self) -> dict[str, Any]:
        return {"records": len(self.values), "identities": len(self.by_identity), "fields": list(WebUrl.__dataclass_fields__)}


def load_urls(path: str | Path) -> WebUrlCollection:
    return WebUrlCollection.load_json(Path(path))


def save_urls(values: WebUrlCollection, path: str | Path) -> Path:
    return values.save_json(Path(path))


def build_urls(**values: Any) -> WebUrl:
    return WebUrl.from_dict(values)
